...
import com.elysiasilly.calvariae.common.block.neo.special.LanternPasteBlock;
...
public class LanternBulbSliceEntity extends ThrowableItemProjectile {
    ...
    public LanternBulbSliceEntity(Level pLevel, LivingEntity pShooter) {
        super(CEntities.LANTERN_BULB_SLICE.get(), pShooter, pLevel);
    }
    ...
    public LanternBulbSliceEntity(Level pLevel, double pX, double pY, double pZ, float damage) {
        super(CEntities.LANTERN_BULB_SLICE.get(), pX, pY, pZ, pLevel);
        DAMAGE = damage;
    }

    @Override
    protected @NotNull Item getDefaultItem() {
        return CItems.LANTERN_BULB_SLICE.get();
    }
    ...
    protected void onHitBlock(@NotNull BlockHitResult result) {
        ...
        if(!level.isClientSide && canBeReplaced && isFaceSturdy) {
            ...
            level.setBlockAndUpdate(adjustedPos, CBlocks.LANTERN_PASTE.get().defaultBlockState().setValue(LanternPasteBlock.FACING, result.getDirection()).setValue(LanternPasteBlock.WATERLOGGED, flag));
            this.discard();
        }
    }
