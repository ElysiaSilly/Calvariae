...
public class LanternKelpBlock extends CMGrowingPlantHeadBlock implements LiquidBlockContainer {
    ...
    protected Block getFruitBlock() {
        return CBlocks.LANTERN_KELP_BULB.get();
    }

    @Override
    protected Block getBodyBlock() {
        return CBlocks.LANTERN_KELP_BODY.get();
    }
    ...
    protected void randomTick(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
        ...
        if(canGrowInto && canCropGrow) {
            if (state.getValue(AGE) == maxAge() && bearsFruit()) {
                ...
                if(chanceForRot == 2) {
                    level.setBlockAndUpdate(posGrowthDirection, CBlocks.ROTTING_LANTERN_KELP_BULB.get().defaultBlockState());
                } else {
                    level.setBlockAndUpdate(posGrowthDirection, CBlocks.LANTERN_KELP_BULB.get().defaultBlockState());
                }
    ...
    protected BlockState updateShape(BlockState state, Direction facing, BlockState facingState, LevelAccessor level, BlockPos currentPos, BlockPos facingPos) {
        ...
        if(bearsFruit() && facing == GROWTH_DIRECTION) {
            if(facingState.is(CBlocks.LANTERN_KELP_BULB.get()) || facingState.is(CBlocks.ROTTING_LANTERN_KELP_BULB.get())) {
    ...
    protected void tick(BlockState state, ServerLevel level, BlockPos pos, RandomSource randomSource) {
        if (!state.canSurvive(level, pos)) {
            ...
            if(bearsFruit()) {
                level.scheduleTick(pos.relative(GROWTH_DIRECTION), CBlocks.LANTERN_KELP_BULB.get(), 1);
                level.scheduleTick(pos.relative(GROWTH_DIRECTION), CBlocks.ROTTING_LANTERN_KELP_BULB.get(), 1);
            }
        }
    }
}
